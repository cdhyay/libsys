...
object DataPartition {
  def main (args: Array[String]) {
    ...
    //----添加自定义词典----
    val dicfile:String = args(0) + "ExtendDic.txt" //ExtendDic为一个文本文件的名字，里面每一行存放一个词
    ...
    //自定义添加停用词
    val stopWordFile:String = args(0)+"stopword.dic"
    ...
    val paper_id_abstract: String = args(0) + "/paper.xlsx"
    ...
    splited.saveAsTextFile(args(1) + "/partedresult")
